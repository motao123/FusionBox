...
本批 Linux 隔离验证：42 项基础检查、67 项行为测试全部通过，零跳过；不触碰线上 SSH、DNS、sysctl、防火墙或现有容器卷。外部上游安装器、真实 ACME/Cloudflare/Telegram 未验证。TG token 参数暴露、流量重试、CF 分区域状态和远程定时备份加固仍待后续批次。
