...
- **上游 URL 存活巡检（P2）**：新增 `scripts/upstream_url_audit.py`——从双语语言包与
  install.sh 提取全部第三方 URL（当前 **21 个**，基线下限 15，防止提取口径悄悄坏掉），
  GET 跟随重定向逐个探活（超时 15s、瞬时错误重试 1 次），任一不可达即失败。
  `--list` 离线提取模式供快闸门使用——run_checks 遵守"无 root、无 Docker、无网络"的
  设计承诺，探活本身只在 GitHub Actions（海外出口）联网执行；CNB 出口对部分域名
  不稳定，有意不接入并在 `.cnb.yml` 注明原因。基线实测：验证服务器 21/21 全部 200。
  `src/modules/` 内的工具类 URL（宝塔/1Panel/reinstall 等）暂不纳入——部分域名按地域
  可达性波动大，会产生假红，待有分位观测再议
- **卸载收尾（P2）**：`self_uninstall` 末尾 `rmdir` 收掉只剩空壳的主体目录
  （`/etc/fusionbox` 与其 `configs/`，目录非空则保留、不碰用户数据）；容器实测卸载后
  主体目录不再残留
- **文档口径（P2）**：README 安装命令注明"命令本身需要系统已带 curl 或 wget"（裸系统
  上 `bash <(curl 失败)` 会静默退出 0 的通病，文档明示前提）；「当前真机口径（v1.43.0）」
  改为「历史基线（v1.43.0）」并指向当前静态门禁口径行，消除评估者对版本基线的误读
- **验收欠账清偿 + 完整回归连抓三个问题**：服务器补跑完整套件（bash 229 项 + Python
  764 项），①外部引用策略检查（刻意的品牌隔离设计，快闸门 201 项不含此项）当场抓到
  v1.43.6 的 CHANGELOG 违规提及同类工具箱项目名——按策略改写为"同类聚合工具箱"，
  未给白名单开口子；②受管应用市场夹具 19 项失败定位为环境缺件（新验证服务器装
  docker.io 未带 compose v2 插件，安装 docker-compose-v2 后归零，非代码回归）；
  ③安装器测试抓到真行为缺陷——curl 不可用（CI 与断网系统常见场景）且探针误判在线时，
  全部下载路径失败后会直接退出而不回落本地安装；现下载链在 main/镜像全失败且本地树
  存在时回落 `_do_local_install`，另将"GitHub 不可达 + 本地树存在"的分支改为本地优先
  （与镜像引导文档口径一致，引导装出的已校验 tarball 不再二次下载）。另一条署名类
  报警为会话导出文件混入服务器副本所致（副本无 .git 时策略按目录扫描），已清理
- **门禁 200→201**：新增断言 h——URL 巡检脚本离线提取口径正常（`--list`）+ 已接线
  GitHub 流水线（release.yml 含探活步骤）
- **验证口径**：验证服务器（Ubuntu 24.04）：完整套件零失败（覆盖 v1.43.4-6 全部改动）、
  静态门禁 **201/201**、在线 URL 探活 21/21 存活、容器实测卸载后主体目录清空；
  本地 Windows 口径 197/4（fcntl 链 + 3 个 docker 消息检查为平台伪影）

## v1.43.6 系统一键重装（DD）：集成 bin456789/reinstall，从严安全边界

- **背景**：同类聚合工具箱对标分析（2026-09-25）后，维护者拍板集成 DD 重装能力。上游选用
  [bin456789/reinstall](https://github.com/bin456789/reinstall)：13.3k 星、GPL-3.0、
  2026-09 仍在持续维护（对比同类聚合工具箱普遍 2023-12 后已停更）；FB 此前审计的结论是
