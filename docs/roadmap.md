...
### A2 OpenSSH 候选版本切换与回滚 — G20 · **v1.37.1 已完成**

> **状态（2026-09-21，v1.37.1）**：`switch` 与 `rollback` 已实现并真机验证（`tests/acceptance/openssh_switch.sh` 32/32）：门禁四项、策略漂移默认拒绝并要求显式接受、原子替换 + reload、**独立看门狗自动回滚**、`--dry-run`、`status` 审计。真机验证还修掉两个只有真实产物才会暴露的问题（OpenSSH 10.x `sshd -T` 保留大小写使既有独立登录检查必然失败；reload 后立刻探测的竞态导致误判回滚）。**唯一未变的边界**：切换事务的生效对象是容器里的 sshd；在宿主机（唯一访问路径）上执行生产切换仍以带外通道为前提。
...
### 批次 1（P0，不依赖任何新条件）→ **已完成：v1.37.0（A1）+ v1.37.1（A2）**
