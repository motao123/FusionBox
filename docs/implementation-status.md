...
2026-09-25 本轮（1.43.7）为 v1.43.6 终审（四类用户视角全量复审，P0=P1=0、5 项 P2）的跟进批次，按优先级逐项落地：**①上游 URL 存活巡检**——新增 `scripts/upstream_url_audit.py`（提取双语语言包 + install.sh 的第三方 URL，当前 21 个、基线下限 15；GET 跟随重定向探活，超时 15s 重试 1 次；`--list` 离线模式供快闸门），接入 GitHub Actions 流水线联网执行；CNB 侧因出口对部分域名不稳定有意不接入（`.cnb.yml` 注明原因）；基线实测验证服务器 21/21 全部 200。**②卸载收尾**——`self_uninstall` 末尾 `rmdir` 收掉只剩空壳的主体目录（非空保留），容器实测不再残留。**③文档口径**——安装命令注明需 curl/wget（裸系统 `bash <(curl 失败)` 静默退 0 的通病）；「当前真机口径（v1.43.0）」改「历史基线」并指向当前口径行。**④验收欠账清偿**——服务器补跑完整套件（bash 229 + Python 764）覆盖 v1.43.4-6 全部改动，连抓三个问题：①外部引用策略（刻意的品牌隔离设计，quick gate 201 项不含此项）当场抓到 v1.43.6 CHANGELOG 违规提及同类工具箱项目名，按策略改写未开白名单口子；②受管市场夹具 19 项失败为环境缺件（新服务器 docker.io 未带 compose v2，装 docker-compose-v2 后归零）；③安装器真行为缺陷——curl 不可用且探针判在线时全部下载路径失败后直接退出，现下载链全失败且本地树存在时回落 `_do_local_install`，另将「GitHub 不可达 + 本地树存在」改为本地优先（与镜像引导文档一致）。另一条署名类报警为会话导出文件混入服务器副本所致（副本无 .git 时策略按目录扫描，该文件不在任何授权名单内），已清理并加入打包排除名单。门禁 **200→201**（断言 h：URL 巡检离线提取 + CI 接线）。验证口径：完整套件零失败、静态门禁服务器 201/201、URL 探活 21/21、容器卸载干净度实测；本地 Windows 197/4。

2026-09-25 本轮（1.43.6）新增**系统一键重装（DD）**，集成 [bin456789/reinstall](https://github.com/bin456789/reinstall)（13.3k 星、GPL-3.0、持续维护）：`src/modules/system.sh` 新增 `system_reinstall`（分发 `fusionbox system reinstall`，工具菜单第 12 项），流程为 TTY 门禁（`[[ ! -t 0 ]]` 拒绝，exit 2）→ `systemd-detect-virt` 虚拟化守卫（openvz/lxc/docker/podman/systemd-nspawn 拒绝）→ 三重确认（风险告知/输入 YES/最终 YES）→ 目标选择（debian 12/13、ubuntu 22.04/24.04、alpine 3.22、自定义 DD 镜像 URL）→ 密码（手输二次确认或自动生成 20 位，DD 模式跳过——上游 `--password` 在 DD 模式仅用于安装期观察日志）→ 双源下载上游脚本（GitHub raw → CNB `-/git/raw/`，后者经用户提供的国内源实测返回真实内容）→ 展示 SHA256 指纹（滚动发布不锁版本，诚实口径）→ 最终确认后执行 → 写入引导成功提示重启（y/N）→ 日志记录（密码脱敏）。上游失败提示未做更改可重试。语言包 3250→**3282**（MSG_SYS_2170-2201，32 键中英对等，EN 包避开 CJK 标点）；门禁 **199→200**（断言 g：函数/TTY 门禁/虚拟化检测/上游 URL/CNB 源五项静态 grep）；README 双语补系统工具能力行与 `system reinstall` 命令行。验证口径（诚实边界：重装主流程会抹盘，无法安全实测）：容器实测虚拟化守卫拒绝、非 TTY 拒绝（exit 2）、双源下载可达、上游 README CLI/环境要求逐项核对；本地套件与服务器静态门禁全绿；真机重装效果以上游自身测试与用户实践背书。

2026-09-25 本轮（1.43.5）补齐更新路径的镜像后备，使安装与更新共用同一套镜像：v1.43.4 给安装器加了 CNB 镜像后备后，实测发现更新路径（`fusionbox update` / `self_update_cron`）仍只走 GitHub、屏蔽 GitHub 的机器更新会失败。`fusion.sh` 新增 `_update_from_mirror`（`FUSION_MIRROR` 常量与安装器同源，环境变量可覆盖）：307 Location 匿名发现最新 tag → Release 资产 + SHA256SUMS 下载并同源校验 → 并入既有"已校验 Release 包"安装分支（版本比较、降级保护、`_update_show_notes` 全部复用）。**静默尝试、零新增语言包键**——更新由运行中的旧版 fusion.sh 执行，新增键会在"新 fusion.sh + 旧语言包"窗口打印裸键名；镜像最新版与当前一致时提前返回"已是最新"（复用 `MSG_MAIN_0008`），避免 cron 场景整包空下载。回落顺序：GitHub Release → CNB 镜像 Release → main 快照（无校验，既有提示）→ 报错。门禁 **198→199**（新增断言：更新镜像后备存在）。验证口径：屏蔽 GitHub 的裸容器实测镜像更新链路（v1.43.4 经镜像装出 → 新 fusion.sh 覆盖运行侧 → `fusionbox update` 走镜像升级 → 升级说明打印）、同版本免下载场景、镜像与 GitHub 资产 sha256 双端一致；静态门禁服务器 **199/199**。

2026-09-25 本轮（1.43.4）为用户视角审计的修复批次，按 P0→P2 逐项落地：**P0 服务器凭据**由维护者本人更换（旧端口/口令实测已不可达，闭环）；**P1 安装镜像后备**——实测发现 CNB 匿名 `-/raw/` 与 `-/archive/` 返回的是 HTTP 200 的软 404 HTML 页（不能用作下载源），仅 Release 资产真实可用且与 GitHub 逐字节一致（sha256 双端比对一致），`-/releases/latest` 以 307 Location 匿名暴露最新 tag，据此 install.sh 新增 `_download_mirror`（`FUSION_MIRROR` 可覆盖，默认 CNB）：GitHub 探测（curl→wget→`/dev/tcp`，镜像主机用 bash 内建 `/dev/tcp` 探测，裸系统无 curl 也能判）失败时照常装依赖后走镜像 Release 下载并过 SHA256SUMS 校验；GitHub 可达但 Release 下载/校验失败时同样回落镜像；校验逻辑抽取为 `_verify_downloaded` 双路径共用，新增 `MSG_INST_0035`（语言包 3249→3250，install.sh 内置表同步）；**P1 FUSION_LANG 优先级**——`common.sh::_load_config` 原先 env 先设、config 后覆盖，而安装器必写 `general.lang`（哪怕 `auto`），导致 docs/i18n.md 承诺的 `FUSION_LANG=en` 单次覆盖在所有已安装系统上失效（真机夹具复现：输出仍中文），改为显式配置 > 环境变量 > `auto`（跟随 `$LANG`）；**P2 日志静默降级**——`_init_log` 的 `mkdir` 失败时置 `F_LOG_OK=0` 静默停用，`_log_write` 加守卫与 `2>/dev/null`，HOME 不可写（容器内 nobody）的帮助输出 stderr 从 4 行噪声降为 0（真机实测）；**P1 文档勘误**——README 双语"完整 Docker 卸载尚未实现"为过期假话（`panels_docker_uninstall` 自 c5280aa/G30 起是含软件包卸载与数据擦除选项的完整实现，命令参考同文件早已列出 `panels docker uninstall`），按实际能力订正；**P2 README.en 净化**——品牌名改官方英文（宝塔→BT Panel、哪吒监控→Nezha monitoring、棉花云→Mianhua Cloud，与 en.sh 译法一致）、命令占位符改英文（`<name>`/`<port>`/`<keyword>`/`<app>`），正文中文残留清零（`CJK_ALLOW` 白名单清空）；双语闸门引入**占位符归一化**（命令行与行内代码比对前把 `<...>` 统一为 `<#>`，两边可各用目标语言书写占位符，其余仍逐字），负向测试复跑（删命令/命令本体错字仍被抓住）；README 双语各补 CNB 镜像手动安装 `<details>` 块；**依赖安装自动重试**——24.04 裸容器真机安装当场撞上 security 池与索引瞬时不同步（404 + `set -e` 裸死 RC=100），各包管理器分支改为失败后 `_ensure_dep_index --force` 强制刷索引重试一次、仍失败才报 `MSG_INST_0016`。门禁 **193→198**：新增 5 断言（FUSION_LANG 打破 auto / 显式 config 仍优先 / 日志静默降级 / 安装器镜像后备存在 / 依赖安装失败自动重试），run_checks.sh 加 Windows 环境提示（自动 `PYTHONUTF8=1`，fcntl 类假红以 Linux 为准），tests/README.md（不入库）记录运行口径。验证口径：新验证服务器（Ubuntu 24.04）Docker 隔离，Ubuntu 22.04 裸容器三条安装路径（GitHub 正常 / `/etc/hosts` 屏蔽 GitHub 后自动镜像 / 分支树本地离线安装）+ 24.04 裸容器真机安装 + 一次性失败包装器确证重试路径，`FUSION_LANG` 夹具矩阵（auto+en→en、en+zh_CN→en）、nobody 帮助 stderr=0、1.43.3→1.43.4 端到端升级并确认升级说明通道首次可见；静态门禁服务器 **198/198**。

2026-09-24 本轮（1.43.3）为安装体验与面板清单收口：安装器语言默认值与运行时 auto 规则对齐（`LANG` 未设置或 `C.UTF-8` 不再装出英文界面），交互式安装完成后直接 `exec` 主菜单（管道式安装仍只打印用法），新增 `MSG_INST_0034` 并同步写入 install.sh 内置表以覆盖"新安装脚本 + 旧发布包"的窗口；新增 `fb` / `FB` 快捷命令（与 `fusionbox` 同为指向 `fusion.sh` 的软链接，创建在 `fusion_deploy` 提交之后，安装与更新两条路径都补齐，外部同名命令不覆盖，卸载对称删除）；面板清单换血——加 1Panel（官方 v2 quick_start）、删 Aapanel（四个语言包键原位改写，键数与菜单编号不变）、宝塔安装携带安装码（该码已随公开仓库发布，是否改法待维护者定）；升级说明通道 `docs/release-notes.md` 铺好（PR #38，CI 强制"顶部小节版本 == version.txt"）——发布后端到端实测纠正了一处错误说法：升级提示由正在运行的旧版 fusion.sh 打印，故本版更新看到的仍是旧格式，新通道自下一版起生效；同时修掉 `self_update` 加载机器上旧 deploy.sh 的问题（真机实测 `fb`/`FB` 在升到本版时未被创建），改为校验后加载刚下载的那份；修正 `MSG_NET_0152` 对 TcpQuality 名不符实的描述，并核实 NodeQuality / TcpQuality 本已在评测矩阵内。验证口径：真机（Ubuntu 22.04 / Docker 29.8.1 / nginx 1.18.0）隔离 base 跑真实 `fusion_deploy` 覆盖三种情形并实跑 `fb version`；四个外部 URL 实测可达；CI 静态闸门全绿；发布后在服务器补跑 193 项快闸门（**193/193**）与完整回归（RC=0），并当场抓到、修正 3 个陈旧检查（详见 CHANGELOG 的 v1.43.3 段）。
...
2026-09-23 本轮（1.43.1）补两处文案与工具链缺口：`MARKET_APPS` 最后一条硬编码条目（Warp）改走语言包，语言包 **3247 → 3248 键**（中英逐键对等）；`scripts/i18n_audit.py` 新增**数据数组抽取棘轮**（5 个约定全量键式的表出现任何字面量条目即失败，与语种无关）+ 中文兜底规则，堵住 `--coverage` 只看文案出口、看不见数组条目的盲区。另新增 `README.en.md` 全文英译与 `scripts/docs_readme_gate.py`（中英结构同步闸门）、`scripts/site_facts.py`（主页数字派生校验），三者均已接入 GitHub 与 CNB 两条流水线。**本轮只动文案与工具链，未重跑真机验收**，真机口径沿用下方 1.43.0 的结果。
