...
v1.8.0 自备 PEM HTTPS：显式 cert/key 路径、OpenSSL 解析/精确 SAN/有效期/公钥匹配，私钥归属与 0600/0400、拒绝链接和不安全路径。不复制/删除私钥、不签发、不自动续期。默认 HTTP 重定向，可显式关闭；TLS enable/status/disable 沿用自有配置与事务回滚，双重失败保留 journal。更新/卸载 503/重装保留 TLS。状态校验本地文件不代表公共 CA 信任、链验证或公网可达性。
...
| G59 | 一键域名访问（应用=反代+证书一条龙） | 部分实现 | v1.8.0 自有宿主 HTTP/自备 PEM HTTPS、SAN/有效期/密钥校验与重载回滚；真实 ACME 后续 |
