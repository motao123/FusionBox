...
仍待处理：TG token argv、流量通知重试、CF 每区域状态、远程定时备份加固。上游安装器、真实 ACME/CF/TG 仍未验证；不是全部 69 项完成声明。下方 v1.4.0 与 Windows 记录为历史验证，不替代本批 Linux 结果。
...
| G17 | 系统备份范围扩展 + 备份管理 | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
...
| G26 | 容器管理增强（进容器/日志/占用/详细信息） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G27 | 容器名级端口开关（按容器+宿主 IP 生成规则） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G28 | Docker 备份增强（compose 项目整备 + 自动生成还原脚本） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G29 | Docker 远程迁移（scp 到目标机） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G30 | Docker 一键卸载（清容器/镜像/包/daemon.json） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G31 | Docker 全局状态总览（容器/镜像/网络/卷计数+全列） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
...
| G57 | 市场机制：统一登记（appno.txt 原子写 0600）+ flock 并发锁 + 端口占用探测分配 + 已装检测 + 镜像更新检测 + 卸载清理 | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G58 | 应用访问模式持久化（direct/domain_only 记录并在更新后恢复） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G59 | 一键域名访问（应用=反代+证书一条龙） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G60 | 磁盘空间预检（按应用体积校验 + NAS 路径软链） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
| G61 | 全量备份/还原（/home 打包 + 可 scp 异地） | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
...
| G64 | 集群配置备份/导入导出 | 后续 | 未在本批补齐；需独立设计、实现与隔离验证 |
