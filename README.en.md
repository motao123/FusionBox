...
> Current version **v1.43.2** | full history in [docs/CHANGELOG.md](docs/CHANGELOG.md)
...
- **Security hardening (each scanner finding re-verified, then fixed)**: the telemetry Worker now reads the request body as a capped stream (chunked and HTTP/2 requests used to be fully buffered before the 4096-byte limit was checked); the write endpoint is no longer cross-origin reachable from browsers - the wildcard CORS shrank to the two public read-only endpoints; numeric segments of the `version` field are length-bounded. The monitoring stack template `monitoring.yml` drops its built-in Grafana admin password (it now fails before start when unset) and binds both panels and Prometheus to 127.0.0.1. The site, reverse proxy and ACME TLS configurations generated by the `web` module now emit `X-Content-Type-Options` / `X-Frame-Options` / `Referrer-Policy`
- **Defaults no longer expose admin surfaces**: the 6 one-click apps where the first visitor becomes administrator or completes the setup (Halo, KodExplorer, LinkStack, Uptime Kuma, Vaultwarden, Memos) now bind to 127.0.0.1 by default, and the printed hint names the two ways to expose them deliberately (reverse proxy with TLS, or an SSH tunnel); Vaultwarden no longer allows public registration
- **New artifact assertion**: `scripts/deploy_exposure_audit.py` requires every published port in a shipped compose file to be either loopback-bound or annotated in place with a `# fb-expose` reason comment, and every credential-like environment variable to be resolved at runtime. The scanner reported one such port; the assertion located **25** in this repository and each is now explicitly accounted for
- **Bug fix**: the Apple CMS deployment in the `web` module unconditionally overwrote its primary compose file with the fallback one, so the dedicated `maccms` image was never used; the fallback now applies only after `docker compose up` fails
- **Verification scope**: the Worker suite passes **8/8** with `tsc --noEmit` clean, and the static gates (i18n 3248 keys equal key for key, README bilingual sync, derived homepage figures, port/credential ratchet, syntax and Python artifacts) are green locally. **Real-machine acceptance was not re-run this round**; the port bindings and nginx configuration changes still need confirmation on the verification server
